...
function Education({ education }) {
  ...
  return (
    <div className="max-md:flex max-md:flex-col-reverse max-md:space-y-1 max-md:space-y-reverse md:flex md:space-x-4  md:items-baseline max-md:p-4 max-md:rounded-lg max-md:hover:bg-app-dark max-md:hover:bg-opacity-20">
      <p className="font-light text-slate-400 text-sm">{`${getMonthYear(
        start
      )} - ${getMonthYear(end)}`}</p>
      <div className="md:flex-1 md:px-8 md:py-8 md:rounded-lg md:hover:bg-app-dark md:hover:bg-opacity-20">
        <h1 className="text-2xl ">{name}</h1>
        <p className="mt-2">
          <span className="text-lg text-app-green ">{school}</span> {" - "}
